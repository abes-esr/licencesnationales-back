...
node {
    ...
    def artifactoryBuildName = "licencesnationales"
    def modulesNames = ["web", "batch"]

    // Definition du module web
    def backApplicationFileName = "licencesnationales"
    def backTargetDir = "/usr/local/tomcat9-licencesNationales/webapps/"
    def backServiceName = "tomcat9-licencesNationales.service"

    // Definition du module batch
    def batchTargetDir = "/home/batch/licencesnationales/"
                ...
                try {
                    ...
                    if ("${candidateModules[moduleIndex]}" == 'web') {
                        withCredentials([
                            ...
                            newconfig = newconfig.replaceAll("google.fr.abes.licencesnationales.recaptcha.key.site=*", "google.fr.abes.licencesnationales.web.recaptcha.key.site=${googleRecaptchaKey}")
                            newconfig = newconfig.replaceAll("google.fr.abes.licencesnationales.recaptcha.key.secret=*", "google.fr.abes.licencesnationales.web.recaptcha.key.secret=${googleRecaptchaSecret}")
                            newconfig = newconfig.replaceAll("google.fr.abes.licencesnationales.recaptcha.key.threshold=*", "google.fr.abes.licencesnationales.web.recaptcha.key.threshold=${googleRecaptchaThreshold}")
